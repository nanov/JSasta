...
char* lsp_handle_definition(LSPServer* server, const char* params) {
    ...
    // Lock to access snapshot
    pthread_mutex_lock(&doc->swap_mutex);
    if (!doc->main_snapshot || !doc->main_snapshot->code_index) {
    ...
    PositionEntry* entry = code_index_find_at_position(doc->main_snapshot->code_index, filename, 
...
char* lsp_handle_references(LSPServer* server, const char* params) {
    ...
    for (int i = 0; i < doc->main_snapshot->code_index->position_count; i++) {
        PositionEntry* pos = &doc->main_snapshot->code_index->positions[i];
...
void lsp_handle_did_open(LSPServer* server, const char* params) {
    ...
    if (!uri || !text) {
        ...
        fprintf(stderr, "[LSP] Missing uri or text in didOpen\n");
        json_value_free(root);
        return;
    }
    
    LSP_LOG("Document opened: %s (version %d, language: %s)", uri, version, language_id ? language_id : "unknown");
    fprintf(stderr, "[LSP] Document opened: %s (version %d)\n", uri, version);
    
    // Open document
    LSPDocument* doc = lsp_document_open(server, uri, language_id, version, text);
    LSP_LOG("Document opened successfully, getting diagnostics");
    
    // Send diagnostics from the initial snapshot
    LSPDiagnostic* diagnostics;
    int count;
    pthread_mutex_lock(&doc->swap_mutex);
    lsp_document_get_diagnostics(doc->main_snapshot, &diagnostics, &count);
    pthread_mutex_unlock(&doc->swap_mutex);
    
    LSP_LOG("Creating diagnostics notification with %d diagnostics", count);
    ...
    LSP_LOG("Sending diagnostics notification: %s", notification);
    lsp_write_message(notification);
    LSP_LOG("Diagnostics sent");
...
void lsp_handle_did_change(LSPServer* server, const char* params) {
    ...
    JSONValue* root = json_parse(params);
    if (!root) {
        fprintf(stderr, "[LSP] Failed to parse didChange params\n");
        return;
    }
    ...
    // Get the new text (full document sync)
    ...
    fprintf(stderr, "[LSP] Document changed: %s (version %d)\n", uri, version);
    
    // Update document - this will schedule a debounced parse
    lsp_document_update(server, uri, version, text);
...
void lsp_handle_did_close(LSPServer* server, const char* params) {
    ...
    JSONValue* root = json_parse(params);
    if (!root) {
        fprintf(stderr, "[LSP] Failed to parse didClose params\n");
        return;
    }
