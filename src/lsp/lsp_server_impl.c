...
static void lsp_document_reparse(LSPServer *server, LSPDocument *doc,
                                 const char *uri);
...
void analysis_work_free(AnalysisWork *work) {
	...
	if (work->ast)
		ast_free(work->ast);
	// Note: Don't free work->symbols here - it's owned by the AST and freed by
	// ast_free()
	if (work->type_ctx)
		type_context_free(work->type_ctx);
	if (work->diagnostics)
		diagnostic_context_free(work->diagnostics);
...
AnalysisWork *analysis_work_parse(const char *content, const char *uri,
                                  const char *filename) {
	...
	work->diagnostics =
	    diagnostic_context_create_with_mode(DIAG_MODE_COLLECT, NULL);
	...
	// The logger has global state that accumulates errors, but parser_parse()
	// checks logger_has_errors() and returns NULL if true. We need to reset it
	// for each document.
	logger_reset_error_count();

	// Parse
	Parser *parser =
	    parser_create(content, filename, work->type_ctx, work->diagnostics);
...
static void lsp_document_reparse(LSPServer *server, LSPDocument *doc,
	...
	if (work->ast) {
		...
		LSP_LOG("Code index %s with %d positions", old_index ? "rebuilt" : "built",
...
static void *diagnostic_worker_thread(void *arg) {
	...
	while (server->worker_running) {
		...
		type_inference_with_diagnostics(work->ast, work->symbols, work->type_ctx,
		                                work->diagnostics);

		LSP_LOG("Type inference complete, errors: %d, warnings: %d",
		        work->diagnostics->error_count, work->diagnostics->warning_count);
		...
		if (work->diagnostics) {
			...
			if (count > 0) {
				...
				while (diag_msg) {
					...
					int line = (diag_msg->location.line > 0)
					               ? (int)diag_msg->location.line - 1
					               : 0;
					int col = (diag_msg->location.column > 0)
					              ? (int)diag_msg->location.column - 1
					              : 0;
					...
					diag->severity = (diag_msg->severity == DIAG_ERROR)
		...
		char *diag_params =
		    lsp_create_diagnostics_notification(work->uri, diagnostics, count);
		char *notification = lsp_serialize_notification(
		...
		LSP_LOG("Worker finished processing, stored completed work for code index "
		        "rebuild");
	}
...
LSPServer *lsp_server_create(void) {
	...
	pthread_create(&server->worker_thread, NULL, diagnostic_worker_thread,
	               server);
...
void lsp_server_free(LSPServer *server) {
	...
	for (int i = 0; i < server->document_count; i++) {
		...
		AnalysisWork *completed =
		    atomic_load(&server->documents[i]->completed_work);
		if (completed) {
...
void lsp_server_run(LSPServer *server) {
	...
	while (!server->shutdown_requested) {
		...
		LspJsonMessage msg = {0};
		if (lsp_read_json_message(&msg) != 0) {
		    fprintf(stderr, "[LSP] Failed to read message, exiting\n");
		    break;
		}

		if (msg.message_type == LSP_JSON_MSG_REQUEST) {
			fprintf(stderr, "[LSP] Request: %s (id=%lld)\n",
			        msg.notification_or_request.method_str,
			        (long long)msg.notification_or_request.id);

			char *response_json = NULL;

			switch (msg.notification_or_request.method_type) {
			case LSP_METHOD_INITIALIZE: {
				char *result = lsp_handle_initialize(server, &msg.notification_or_request.params.initialize);
			...
			case LSP_METHOD_TEXTDOCUMENT_HOVER: {
				char *result = lsp_handle_hover(server, &msg.notification_or_request.params.hover);
				response_json = lsp_serialize_response(msg.notification_or_request.id, result);
				free(result);
			} break;
			case LSP_METHOD_TEXTDOCUMENT_COMPLETION: {
				char *result = lsp_handle_completion( server, &msg.notification_or_request.params.completion);
				response_json = lsp_serialize_response(msg.notification_or_request.id, result);
				free(result);
			} break;
			case LSP_METHOD_TEXTDOCUMENT_DEFINITION: {
				char *result = lsp_handle_definition( server, &msg.notification_or_request.params.definition);
				response_json = lsp_serialize_response(msg.notification_or_request.id, result);
				free(result);
			} break;
			...
			default:
				response_json = lsp_serialize_error(msg.notification_or_request.id, 404, "Method not found");
			}
		...
		} else if (msg.message_type == LSP_JSON_MSG_NOTIFICATION) {
			fprintf(stderr, "[LSP] Notification: %s\n",
			        msg.notification_or_request.method_str);

			switch (msg.notification_or_request.method_type) {

			case LSP_METHOD_INITIALIZE: lsp_handle_initialized(server); break;
			case LSP_METHOD_EXIT: lsp_handle_exit(server); break;
			case LSP_METHOD_TEXTDOCUMENT_DID_OPEN:
				lsp_handle_did_open(server, &msg.notification_or_request.params.didOpen); // msg->notification.params);
				break;
			case LSP_METHOD_TEXTDOCUMENT_DID_CHANGE:
				lsp_handle_did_change(server, &msg.notification_or_request.params.didChange);
				break;
			case LSP_METHOD_TEXTDOCUMENT_DID_CLOSE:
				lsp_handle_did_close(server, &msg.notification_or_request.params.didClose);
				break;
			case LSP_METHOD_TEXTDOCUMENT_DID_SAVE:
				lsp_handle_did_save(server, &msg.notification_or_request.params.didSave);
				break;
			default: break;
			}
		}
		lsp_json_inner_free_message(&msg);
	}
...
CodeIndex *lsp_document_get_code_index(LSPDocument *doc) {
	...
	if (completed) {
		...
		analysis_work_free(completed);
		LSP_LOG("Code index rebuilt with %d positions",
		        doc->code_index->position_count);
	}
...
LSPDocument *lsp_document_open(LSPServer *server, const char *uri,
                               const char *language_id, int version,
                               const char *text) {
	...
	if (server->document_count >= server->document_capacity) {
		...
		server->documents = realloc(
		    server->documents, sizeof(LSPDocument *) * server->document_capacity);
	}
...
void lsp_document_update(LSPServer *server,
                         LspJsonDidChangeTextDocumentParams
                             *params) { // const char* uri, int version, const
	                                      // TextRange* range, const char* text) {
	fprintf(stderr, "[LSP] updateding unopened document: %s\n",
	        params->textDocument.uri);
	if (params->changes_length < 1)
		return;

	LSPDocument *doc = lsp_document_find(server, params->textDocument.uri);
	if (!doc) {
	...
	for (size_t i = 0; i < params->changes_length; i++) {
		LspJsonTextDocumentContentChangeEvent c = params->contentChanges[i];
		if (c.has_range) {
			TextRange range = c.range;
			jsa_string_builder_apply_edit(doc->content, &range, c.text);
		} else {
			jsa_string_builder_clear(doc->content);
			jsa_string_builder_append(doc->content, c.text);
			break; // only one shouldbe here
		}
	}
	fprintf(stderr, "[LSP] updateding unopened document: %s\n",
	        params->textDocument.uri);
	...
	*/
	...
	// Parse and queue for type inference
	lsp_document_reparse(server, doc, params->textDocument.uri);
}
...
void lsp_document_get_diagnostics(AnalysisWork *work,
                                  LSPDiagnostic **out_diagnostics,
                                  int *out_count) {
	...
	while (diag_msg) {
		...
		// Convert to 0-based LSP positions, but handle invalid locations (line/col
		// 0) If location is missing/invalid, default to line 0 (first line)
		int line =
		    (diag_msg->location.line > 0) ? (int)diag_msg->location.line - 1 : 0;
		int col = (diag_msg->location.column > 0)
		              ? (int)diag_msg->location.column - 1
		              : 0;
...
static ASTNode *find_identifier_recursive(ASTNode *node, int line,
                                          int character) {
	if (!node)
		return NULL;
	...
	if (node->type == AST_IDENTIFIER && node->loc.line > 0) {
		...
		if (node_line == line && character >= node_col &&
		    character < node_col + name_len) {
			return node;
		}
	}
	...
	case AST_BLOCK:
		for (int i = 0; i < node->program.count; i++) {
			ASTNode *result = find_identifier_recursive(node->program.statements[i],
			                                            line, character);
			if (result)
				return result;
		}
	...
	case AST_IF: {
		ASTNode *result =
		    find_identifier_recursive(node->if_stmt.condition, line, character);
		if (result)
			return result;
		result =
		    find_identifier_recursive(node->if_stmt.then_branch, line, character);
		if (result)
			return result;
		return find_identifier_recursive(node->if_stmt.else_branch, line,
		                                 character);
	}

	case AST_WHILE: {
		ASTNode *result =
		    find_identifier_recursive(node->while_stmt.condition, line, character);
		if (result)
			return result;
	...
	case AST_FOR: {
		ASTNode *result =
		    find_identifier_recursive(node->for_stmt.init, line, character);
		if (result)
			return result;
		result =
		    find_identifier_recursive(node->for_stmt.condition, line, character);
		if (result)
			return result;
	...
	case AST_EXPR_STMT:
		return find_identifier_recursive(node->expr_stmt.expression, line,
		                                 character);

	case AST_BINARY_OP: {
		ASTNode *result =
		    find_identifier_recursive(node->binary_op.left, line, character);
		if (result)
			return result;
	...
	case AST_CALL: {
		ASTNode *result =
		    find_identifier_recursive(node->call.callee, line, character);
		if (result)
			return result;
	...
	case AST_MEMBER_ACCESS:
		return find_identifier_recursive(node->member_access.object, line,
		                                 character);

	case AST_INDEX_ACCESS: {
		ASTNode *result =
		    find_identifier_recursive(node->index_access.object, line, character);
		if (result)
			return result;
	...
	case AST_ARRAY_LITERAL:
		for (int i = 0; i < node->array_literal.count; i++) {
			ASTNode *result = find_identifier_recursive(
			    node->array_literal.elements[i], line, character);
			if (result)
				return result;
		}
		break;
...
ASTNode *lsp_find_identifier_at_position(ASTNode *ast, int line,
                                         int character) {
...
char *lsp_get_hover_info(ASTNode *node, SymbolEntry *symbol) {
	...
	if (symbol && symbol->type_info) {
		...
		snprintf(info, 256, "**%s**\n\nType: `%s`", symbol->name,
		         symbol->type_info->type_name ? symbol->type_info->type_name
		                                      : "unknown");
		return info;
	}
	...
	if (node && node->type_info) {
		...
		snprintf(info, 256, "Type: `%s`",
		         node->type_info->type_name ? node->type_info->type_name
		                                    : "unknown");
		return info;
	}
