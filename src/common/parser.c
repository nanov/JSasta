...
#define PARSE_ERROR(parser, err, ...) do { \
    SourceLocation loc = { \
        .filename = (parser)->filename, \
        .line = (parser)->current_token->line, \
        .column = (parser)->current_token->column \
    }; \
    JSASTA_ERROR((parser)->diagnostics, loc, err, ##__VA_ARGS__); \
} while(0)
...
static bool parser_expect(Parser* parser, TokenType type) {
    if (!parser_match(parser, type)) {
        ...
        JSASTA_ERROR(parser->diagnostics, loc, PE_EXPECTED_TOKEN,
...
static TypeInfo* parse_type_annotation(Parser* parser) {
    ...
    if (!type_info) {
        ...
        while (parser_match(parser, TOKEN_DOT)) {
            ...
            if (!parser_match(parser, TOKEN_IDENTIFIER)) {
                PARSE_ERROR(parser, PE_EXPECTED_PROPERTY_NAME_AFTER_DOT);
                return Type_Unknown;
            }
        ...
        if (strchr(type_path, '.')) {
            ...
            if (parser->type_ctx) {
                ...
                if (!type_info) {
                    PARSE_ERROR(parser, PE_UNKNOWN_TYPE_PATH, type_path);
...
static int parse_field_initializers(Parser* parser, char*** field_names, ASTNode*** field_values, JSastaErrorCode error_code) {
    int capacity = 4;
            ...
            if (is_object_literal) {
                ...
                do {
                    ...
                    if (!parser_match(parser, TOKEN_IDENTIFIER) && !parser_match(parser, TOKEN_STRING)) {
                        PARSE_ERROR(parser, PE_EXPECTED_PROPERTY_NAME);
                        return node;
                    }
    ...
    while (parser_match(parser, TOKEN_LPAREN) || parser_match(parser, TOKEN_DOT) ||
        ...
        if (parser_match(parser, TOKEN_PLUSPLUS) || parser_match(parser, TOKEN_MINUSMINUS)) {
            ...
            if (node->type == AST_IDENTIFIER) {
                ...
                PARSE_ERROR(parser, PE_POSTFIX_ON_NON_LVALUE);
                return node;
            }
        }
        if (parser_match(parser, TOKEN_LBRACKET)) {
            ...
            if (node->type == AST_MEMBER_ACCESS) {
                ...
                if (node->member_access.object->type == AST_IDENTIFIER) {
                    ...
                    PARSE_ERROR(parser, PE_EXPECTED_ENUM_TYPE_IN_PATTERN);
                    return node;
                }
                ...
                int field_count = parse_field_initializers(parser,
                    ...
                    PE_EXPECTED_DOT_AFTER_ENUM_TYPE);
                ...
                int field_count = parse_field_initializers(parser,
                    ...
                    PE_EXPECTED_VARIANT_NAME_IN_PATTERN);
...
static ASTNode* parse_unary(Parser* parser) {
    ...
    if (parser_match(parser, TOKEN_PLUSPLUS) || parser_match(parser, TOKEN_MINUSMINUS)) {
        ...
        if (!target) {
            PARSE_ERROR(parser, PE_EXPECTED_EXPR_AFTER_PREFIX, prefix->prefix_op.op);
            return NULL;
        }
        ...
        while (parser_match(parser, TOKEN_DOT) || parser_match(parser, TOKEN_LBRACKET)) {
            if (parser_match(parser, TOKEN_DOT)) {
                ...
                if (!parser_match(parser, TOKEN_RBRACKET)) {
                    PARSE_ERROR(parser, PE_EXPECTED_CLOSING_BRACKET);
                    return NULL;
                }
                parser_advance(parser);
                target = index;
...
static ASTNode* parse_pattern_match(Parser* parser, ASTNode* expr) {
    ...
    // Parse .Variant
    if (!parser_match(parser, TOKEN_DOT)) {
        PARSE_ERROR(parser, PE_EXPECTED_DOT_AFTER_ENUM_TYPE);
        return node;
    }
    parser_advance(parser);

    if (!parser_match(parser, TOKEN_IDENTIFIER)) {
        PARSE_ERROR(parser, PE_EXPECTED_VARIANT_NAME_IN_PATTERN);
        return node;
    }
    ...
    if (parser_match(parser, TOKEN_LPAREN)) {
        ...
        while (!parser_match(parser, TOKEN_RPAREN) && !parser_match(parser, TOKEN_EOF)) {
            ...
            // Check for binding with let/var/const
                ...
                PARSE_ERROR(parser, PE_EXPECTED_BINDING_KEYWORD);
                free(names);
                free(is_mutable);
                free(is_wildcard);
                return node;
            ...
            if (parser_match(parser, TOKEN_COMMA)) {
                ...
                PARSE_ERROR(parser, PE_EXPECTED_COMMA_OR_PAREN_IN_PATTERN);
                free(names);
                free(is_mutable);
                free(is_wildcard);
                return node;
...
static ASTNode* parse_assignment(Parser* parser) {
    ...
    if (parser_match(parser, TOKEN_PLUS_ASSIGN) || parser_match(parser, TOKEN_MINUS_ASSIGN) ||
        ...
        if (node->type == AST_IDENTIFIER) {
            ...
            PARSE_ERROR(parser, PE_COMPOUND_ASSIGN_REQUIRES_LVALUE);
            return node;
        }
    }
    ...
    if (parser_match(parser, TOKEN_ASSIGN)) {
        ...
        // Handle member assignment: obj.prop = value
            ...
            PARSE_ERROR(parser, PE_INVALID_ASSIGNMENT_TARGET);
            return node;
        }
    }
...
static ASTNode* parse_var_declaration(Parser* parser) {
    ...
    if (!parser_match_identifier_like(parser)) {
        ...
        if (parser->current_token->type >= TOKEN_I8 && parser->current_token->type <= TOKEN_INT) {
            ...
            PARSE_ERROR(parser, PE_EXPECTED_IDENTIFIER_AFTER_VAR);
        }
    ...
    if (parser_match(parser, TOKEN_LBRACKET)) {
        ...
        if (!size_expr) {
            PARSE_ERROR(parser, PE_EXPECTED_ARRAY_SIZE);
            ast_free(node);
            return NULL;
        }
        ...
        if (!parser_match(parser, TOKEN_RBRACKET)) {
            PARSE_ERROR(parser, PE_EXPECTED_CLOSING_BRACKET_AFTER_SIZE);
            ast_free(node);
            return NULL;
        }
        parser_advance(parser);  // consume ']'
    ...
    if (parser_match(parser, TOKEN_ASSIGN)) {
        ...
        if (!node->var_decl.init) {
            PARSE_ERROR(parser, PE_EXPECTED_EXPR_AFTER_EQUALS);
            ast_free(node);
            return NULL;
        }
    } else {
...
static ASTNode* parse_function_declaration(Parser* parser) {
    ...
    node->func_decl.param_locs = (SourceLocation*)malloc(sizeof(SourceLocation) * capacity);
    ...
    if (!parser_match(parser, TOKEN_RPAREN)) {
        do {
            if (node->func_decl.param_count >= capacity) {
                ...
                node->func_decl.param_locs = (SourceLocation*)realloc(node->func_decl.param_locs, sizeof(SourceLocation) * capacity);
                node->func_decl.param_type_hints = (TypeInfo**)realloc(node->func_decl.param_type_hints, sizeof(TypeInfo*) * capacity);
            }
            // Capture parameter location before advancing
            node->func_decl.param_locs[node->func_decl.param_count].filename = parser->filename;
            node->func_decl.param_locs[node->func_decl.param_count].line = parser->current_token->line;
            node->func_decl.param_locs[node->func_decl.param_count].column = parser->current_token->column;
...
static ASTNode* parse_external_function_declaration(Parser* parser) {
    ...
    if (!parser_match(parser, TOKEN_RPAREN)) {
        do {
            ...
            param_loc.filename = parser->filename;
            ...
            if (is_type_keyword) {
                ...
                if (parser->current_token->type == TOKEN_COLON) {
                    ...
                    if (!param_type) {
                        PARSE_ERROR(parser, PE_EXTERNAL_FUNC_NEEDS_TYPE_HINTS);
                    ...
                    if (!param_type) {
                        PARSE_ERROR(parser, PE_UNKNOWN_TYPE_IN_PARAM, first_identifier);
                        free(first_identifier);
                        return node;
                    }
                    // Generate a placeholder name for the parameter
                ...
                PARSE_ERROR(parser, PE_EXPECTED_PARAM_NAME_OR_TYPE);
                return node;
            }

            node->func_decl.params[node->func_decl.param_count] = param_name;
            node->func_decl.param_locs[node->func_decl.param_count] = param_loc;
    ...
    node->func_decl.return_type_hint = parse_type_annotation(parser);
    if (!node->func_decl.return_type_hint) {
        PARSE_ERROR(parser, PE_EXTERNAL_FUNC_NEEDS_RETURN_TYPE);
        return node;
    }
...
static ASTNode* parse_struct_declaration(Parser* parser) {
    ...
    node->struct_decl.property_locs = (SourceLocation*)malloc(sizeof(SourceLocation) * capacity);
    ...
    while (!parser_match(parser, TOKEN_RBRACE) && !parser_match(parser, TOKEN_EOF)) {
        if (node->struct_decl.property_count >= capacity) {
            ...
            node->struct_decl.property_locs = (SourceLocation*)realloc(node->struct_decl.property_locs, sizeof(SourceLocation) * capacity);
        ...
        member_loc.filename = parser->filename;
        member_loc.line = parser->current_token->line;
        member_loc.column = parser->current_token->column;
        ...
        if (parser_match(parser, TOKEN_LPAREN)) {
            ...
            method->func_decl.param_locs = (SourceLocation*)malloc(sizeof(SourceLocation) * param_capacity);
            ...
            while (!parser_match(parser, TOKEN_RPAREN) && !parser_match(parser, TOKEN_EOF)) {
                if (method->func_decl.param_count >= param_capacity) {
                    ...
                    method->func_decl.param_locs = (SourceLocation*)realloc(method->func_decl.param_locs, sizeof(SourceLocation) * param_capacity);
                ...
                if (parser->current_token->type != TOKEN_IDENTIFIER) {
                    PARSE_ERROR(parser, PE_EXPECTED_METHOD_PARAM_NAME);
                    free(member_name);
                    return node;
                }
                ...
                if (!param_type) {
                    PARSE_ERROR(parser, PE_METHOD_PARAM_NEEDS_TYPE, param_name);
                    free(param_name);
                    free(member_name);
                    return node;
                }
                ...
                method->func_decl.param_locs[method->func_decl.param_count] = param_loc;
            ...
            if (!method->func_decl.return_type_hint) {
                PARSE_ERROR(parser, PE_METHOD_NEEDS_RETURN_TYPE, member_name);
                free(member_name);
                return node;
            }
        ...
        if (!prop_type) {
            PARSE_ERROR(parser, PE_STRUCT_PROPERTY_NEEDS_TYPE, member_name);
            free(member_name);
            return node;
        }
        ...
        if (parser_match(parser, TOKEN_LBRACKET)) {
            ...
            if (!array_size_expr) {
                PARSE_ERROR(parser, PE_EXPECTED_ARRAY_SIZE_IN_STRUCT);
                free(member_name);
                return node;
            }
            ...
            if (!parser_match(parser, TOKEN_RBRACKET)) {
                PARSE_ERROR(parser, PE_EXPECTED_CLOSING_BRACKET_IN_STRUCT);
                free(member_name);
                return node;
            }
            parser_advance(parser); // consume ']'
        ...
        if (array_size == 0 && !array_size_expr && type_info_is_array(prop_type)) {
            PARSE_ERROR(parser, PE_STRUCT_ARRAY_NEEDS_SIZE_OR_REF);
            free(member_name);
            return node;
        }
        ...
        if (parser_match(parser, TOKEN_ASSIGN)) {
            ...
            if (parser->current_token->type == TOKEN_NUMBER) {
                ...
                PARSE_ERROR(parser, PE_DEFAULT_VALUE_MUST_BE_LITERAL);
                free(member_name);
                return node;
            }
        ...
        node->struct_decl.property_locs[node->struct_decl.property_count] = member_loc;
...
static ASTNode* parse_enum_declaration(Parser* parser) {
    ...
    // Parse enum name
    if (parser->current_token->type != TOKEN_IDENTIFIER) {
        PARSE_ERROR(parser, PE_EXPECTED_ENUM_NAME);
        return node;
    }
    ...
    while (!parser_match(parser, TOKEN_RBRACE) && !parser_match(parser, TOKEN_EOF)) {
        ...
        // Parse variant name
        if (parser->current_token->type != TOKEN_IDENTIFIER) {
            PARSE_ERROR(parser, PE_EXPECTED_VARIANT_NAME);
            return node;
        }
        ...
        if (parser_match(parser, TOKEN_SEMICOLON)) {
            ...
            if (is_named) {
                ...
                if (!field_types[field_count]) {
                    PARSE_ERROR(parser, PE_FIELD_NEEDS_TYPE_IN_VARIANT, field_names[field_count]);
            ...
            PARSE_ERROR(parser, PE_EXPECTED_SEMICOLON_OR_PAREN_AFTER_VARIANT);
        }
...
static ASTNode* parse_import_declaration(Parser* parser) {
    ...
    if (parser->current_token->type == TOKEN_AT) {
        parser_advance(parser);
        if (parser->current_token->type != TOKEN_IDENTIFIER) {
            PARSE_ERROR(parser, PE_EXPECTED_MODULE_PATH);
            return node;
        }
    ...
    // Get namespace name
    if (parser->current_token->type != TOKEN_IDENTIFIER) {
        PARSE_ERROR(parser, PE_EXPECTED_NAMESPACE_IDENTIFIER);
        return node;
    }
    ...
    if (parser->current_token->type != TOKEN_FROM) {
        PARSE_ERROR(parser, PE_EXPECTED_FROM_KEYWORD);
        return node;
    }
    parser_advance(parser);
...
static ASTNode* parse_export_declaration(Parser* parser) {
    ...
    if (parser_match(parser, TOKEN_FUNCTION)) {
        ...
        PARSE_ERROR(parser, PE_EXPECTED_SEMICOLON_AFTER_IMPORT);
        return node;
    }
...
ASTNode* parser_parse(Parser* parser) {
    ...
    while (!parser_match(parser, TOKEN_EOF)) {
        ...
        if (parser->current_token->line == prev_line &&
            ...
            !parser_match(parser, TOKEN_EOF)) {
            PARSE_ERROR(parser, PE_STUCK_ON_TOKEN,
                    token_type_to_string(parser->current_token->type),
