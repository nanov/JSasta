...
static bool io_format_validate(ASTNode* call_node, TypeContext* type_ctx, DiagnosticContext* diag) {
    ...
    if (call_node->method_call.arg_count < 1) {
        JSASTA_ERROR(diag, call_node->loc, VE_FORMAT_REQUIRES_ARG, func_name);
        return false;
    }

    ASTNode* format_arg = call_node->method_call.args[0];
    if (format_arg->type != AST_STRING) {
        JSASTA_ERROR(diag, format_arg->loc, VE_FORMAT_STRING_MUST_BE_LITERAL, func_name);
        return false;
    }
    ...
    if (!fs) {
        JSASTA_ERROR(diag, format_arg->loc, VE_INVALID_FORMAT_STRING);
        return false;
    }
    ...
    if (fs->placeholder_count != actual_args) {
        if (actual_args > fs->placeholder_count) {
            ...
            JSASTA_WARNING(diag, call_node->loc, VW_FORMAT_EXTRA_ARGS,
                func_name,
                fs->placeholder_count, fs->placeholder_count == 1 ? "" : "s",
                actual_args, actual_args == 1 ? "" : "s");
        } else {
            // Fewer arguments than placeholders - error
            JSASTA_ERROR(diag, call_node->loc, VE_FORMAT_PLACEHOLDER_MISMATCH,
                func_name,
                fs->placeholder_count, fs->placeholder_count == 1 ? "" : "s",
                actual_args, actual_args == 1 ? "" : "s");
...
static bool test_assert_that_validate(ASTNode* call_node, TypeContext* type_ctx, DiagnosticContext* diag) {
    ...
    if (call_node->method_call.arg_count < 1) {
        JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_THAT_REQUIRES_ARG);
        return false;
    }
    ...
    if (condition_arg->type_info != Type_Bool) {
        JSASTA_ERROR(diag, condition_arg->loc, VE_ASSERT_THAT_CONDITION_MUST_BE_BOOL,
    ...
    if (call_node->method_call.arg_count >= 2) {
        ...
        if (msg_arg->type != AST_STRING) {
            JSASTA_ERROR(diag, msg_arg->loc, VE_ASSERT_THAT_MESSAGE_MUST_BE_LITERAL);
            return false;
        }
        ...
        if (!fs) {
            JSASTA_ERROR(diag, msg_arg->loc, VE_ASSERT_THAT_INVALID_FORMAT);
            return false;
        }
        ...
        if (fs->placeholder_count != actual_args) {
            if (actual_args < fs->placeholder_count) {
                JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_THAT_PLACEHOLDER_MISMATCH,
                    fs->placeholder_count, fs->placeholder_count == 1 ? "" : "s",
                    actual_args, actual_args == 1 ? "" : "s");
                format_string_free(fs);
                return false;
            } else {
                JSASTA_WARNING(diag, call_node->loc, VW_ASSERT_THAT_EXTRA_ARGS,
                    fs->placeholder_count, fs->placeholder_count == 1 ? "" : "s",
                    actual_args, actual_args == 1 ? "" : "s");
            }
        }
...
static bool test_assert_equals_validate(ASTNode* call_node, TypeContext* type_ctx, DiagnosticContext* diag) {
    ...
    if (call_node->method_call.arg_count != 2) {
        JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_EQUALS_REQUIRES_TWO_ARGS, "assert_equals");
        return false;
    }
    ...
    if (arg0->type_info != arg1->type_info) {
        JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_EQUALS_TYPE_MISMATCH, "assert_equals");
        return false;
    }
    ...
    if (Trait_Eq) {
        ...
        if (!eq_impl) {
            JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_EQUALS_NEEDS_EQ_TRAIT,
                arg0->type_info->type_name ? arg0->type_info->type_name : "unknown");
            return false;
        }
    }
...
static bool test_assert_not_equals_validate(ASTNode* call_node, TypeContext* type_ctx, DiagnosticContext* diag) {
    ...
    // Validate we have exactly 2 arguments
    if (call_node->method_call.arg_count != 2) {
        JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_NOT_EQUALS_REQUIRES_TWO_ARGS, "assert_not_equals");
        return false;
    }
    ...
    // Check that both arguments have the same type
    if (arg0->type_info != arg1->type_info) {
        JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_NOT_EQUALS_TYPE_MISMATCH, "assert_not_equals");
        return false;
    }
    ...
    if (Trait_Eq) {
        ...
        if (!eq_impl) {
            JSASTA_ERROR(diag, call_node->loc, VE_ASSERT_NOT_EQUALS_NEEDS_EQ_TRAIT,
                arg0->type_info->type_name ? arg0->type_info->type_name : "unknown");
            return false;
        }
    }
